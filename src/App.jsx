...
const ChessApp = lazy(() =>
  import("./components/apps/ChessApp.jsx").then((m) => ({ default: m.ChessApp }))
);
...
function AppContent() {
  ...
  const [wallpaperId, setWallpaperState] = useState(() => {
    if (typeof window === "undefined") return "wallpaper-protagonist";
    return window.localStorage?.getItem(WALLPAPER_STORAGE_KEY) || "wallpaper-protagonist";
  });
...
function Loader({ onComplete }) {
  ...
  useGSAP(() => {
    ...
    gsap.to(progressRef.current, {
      width: "100%",
      duration: 2.2, // Fill almost fully before fading
      ease: "power1.inOut",
    });
  ...
  return (
    <div
      ref={loaderRef}
      className="loader fixed inset-0 z-[500] flex flex-col items-center justify-center bg-black"
    >
      <img
        src="/images/appleicon.png"
        alt=""
        // Force the icon to pure white for the dark boot screen
        className="w-20 h-20 mb-16 brightness-0 invert opacity-100"
        aria-hidden="true"
      />
      {/* Native-style macOS loading bar */}
      <div className="w-56 h-1 bg-[#333333] rounded-full overflow-hidden">
        <div
          ref={progressRef}
          className="h-full bg-[#ffffff] w-0 rounded-full"
        />
      </div>
    </div>
  );
